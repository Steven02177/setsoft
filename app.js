...
resultado = A.intersection(B)`,
    sql:
`-- El INNER JOIN devuelve solo las filas que
-- tienen correspondencia en AMBAS tablas.
SELECT a.id, a.nombre
FROM tabla_a a
INNER JOIN tabla_b b ON a.id = b.id;

-- Equivale a: solo los que están en A y en B.`,
    js:
`const A = new Set([1, 2, 3, 4]);
const B = new Set([3, 4, 5, 6]);
...
resultado = A.difference(B)`,
    sql:
`-- NOT IN excluye de la primera tabla
...
-- Equivale a: los de A que NO están en B.`,
    js:
`const A = new Set([1, 2, 3, 4]);
const B = new Set([3, 4, 5, 6]);
...
resultado = U - A
print(resultado)   # {5, 6, 7, 8}

# El complemento es la diferencia del universo menos A`,
    sql:
`-- Seleccionar todos los del universo
...
-- Equivale a: todos menos los que están en A.`,
    js:
...
console.log([...complemento]); // [5, 6, 7, 8]`
  }
};
...
function calcular() {
  ...
  document.getElementById('codePython').textContent = d.python;
  document.getElementById('codeSQL').textContent    = d.sql;
  document.getElementById('codeJS').textContent     = d.js;
...
function dibujarVenn(A, B, U, res, op) {
  ...
  if (op === 'complemento') {
    ...
    // Círculo A en el centro (lo que NO es el resultado)
    ...
    // Resaltar zona del complemento (fuera del círculo)
    ctx.save();
    ctx.beginPath();
    ...
    // Elementos del complemento (fuera del círculo)
  ...
  // Mostrar elementos en las zonas
...
function descargarResultado() {
  ...
  // Canvas de exportación más grande
  ...
  // Sección de conjuntos (izquierda)
